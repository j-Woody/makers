...
public class PorderController {
	...
	@RequestMapping(value="porderInsert", method=RequestMethod.GET)
	public String porderInsert(int[] ctnum,int buy,Porder porder,Model model) {
		
		System.out.println(porder.getOraddr());
	
		for(int i = 0; i<ctnum.length; i++){
			
			System.out.println(i+" = "+ctnum[i]);
		}
		//System.out.println("넌 왜 안넘어오냐 BUY = "+buy+"oraddr = "+porder.getOraddr()+"ctnum="+ctnum[0]+","+ctnum[1]);
		...
		if(buy == 1) {
			...
			productService.changeCnt(product);
					...
					int total= 0;
					for (int i = 0; i < ctnum.length; i++) {
						cartList.add(cartService.cartGetToNum(ctnum[i]));
					}
					for (Cart c : cartList) {
				/* c.setCtstate(2); */
